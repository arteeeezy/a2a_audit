...
@client.event
async def on_ready():
    print(f"Analyst Bot 上线: {client.user}")
...
async def on_message(message):
    ...
    ws = discord.utils.get(message.guild.channels, name="analysis-workspace")
    if ws:
    ...
    if not result or result.startswith("执行失败"):
        ...
        if error_ch:
            await error_ch.send(f"**[{task_id}] Analyst 执行失败**\n{result}")
        return
...
client.run(os.getenv("ANALYST_TOKEN"))